...
GulfTalent collection uses the public v2 search API for configured destinations in UAE, Saudi Arabia, Qatar, Kuwait, Bahrain and Oman. Country queries are deduplicated across cities; the existing location and matching filters still apply. It requests 25 results per page in relevance order, up to `max_pages`, and includes both direct and aggregated listings. Unlike LinkedIn, this source has no date-range filter. Descriptions retain requirements and advertised salary. Before delivery, the public listing must identify the same job and expose a matching application entry point; missing or blocked pages remain pending. HTTP 403/429 stops GulfTalent collection for that run. No GulfTalent login or copied browser cookies are configured. The adapter was checked against browser-captured API responses; anonymous live API access still needs verification from the deployment host.
