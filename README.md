...
GulfTalent collection uses the public v2 search API for configured destinations in UAE, Saudi Arabia, Qatar, Kuwait, Bahrain and Oman. Each country query includes the configured city IDs when all destinations have known IDs; country-wide destinations retain country-wide coverage. Existing location and matching filters still apply. It requests 25 results per page with date ordering (`search_order=d`), up to `max_pages`, and includes both direct and aggregated listings. The site may show older results despite its Date selection, so collection also enforces `max_job_age_days` (seven days by default). Descriptions retain requirements and advertised salary. Before delivery, the public listing must identify the same job and expose a matching application entry point; missing or blocked pages remain pending. HTTP 403/429 stops GulfTalent collection for that run. No GulfTalent login or copied browser cookies are configured. Search, combined city filters and pagination were verified in local Chrome; the standalone API collector still needs live verification from the deployment host.
...
New candidates send `/start` to begin guided onboarding. `/continue` resumes their saved checklist, and `/status` shows progress and the next question. The flow reviews resume facts and experience, roles and experience limits, destinations and work authorization, timing, Telegram delivery and presentation, then LinkedIn and optional Gmail/tracker connections. Each step has an explicit review or skip action where appropriate. New schedules stay paused until the final review and separate activation confirmation. Previously running profiles are not paused by viewing their status. Changes to reviewed settings require reviewing the affected step again.

The bot announces “Please wait” before reading an uploaded resume, calling Hermes, or checking a connection. Completed questions and previews say “Your turn” and explain whether to reply or confirm. Confirming changed resume facts during the resume step automatically asks the next focused interview question using the saved facts; replaying that confirmation does not launch another interview turn. The candidate still explicitly chooses “Done reviewing” to move on. Failed interview requests offer a retry, and a saved uploaded draft does not need to be uploaded again.

Connection checks distinguish saved account settings, completed consent and verified access. Unavailable Google integration is explained before issuing a link and can be skipped. After a model failure, the bot saves the pending request and explains `/retry`; status and other deterministic commands remain available. Retries use current confirmed settings and cannot confirm changes automatically. `/support` produces a limited report the candidate can share with the owner; it does not give the candidate system access or send their private resume to the owner.
