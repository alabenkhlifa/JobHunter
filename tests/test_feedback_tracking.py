...
def test_owner_feedback_learning_no_longer_adjusts_scores():
    summary = {"by_action": {"skip": 4, "interested": 3},
               "by_reason": {"wrong stack": 2, "strong backend fit": 2}}
    job = {"title": "Senior Backend Engineer", "description": "React nice to have; devops.",
           "tech_required": "java, spring boot", "score": 12}

    learned = scraper.apply_feedback_learning(job, summary)

    assert learned == job
    assert "feedback_adjusted_score" not in learned
