...
Install this as a separate restricted service using the owner's existing Telegram bot and model login. Hermes remains the sole bot poller; its routing plugin forwards candidate updates to a durable local queue before any administrative command or media handler. Keep the owner's Hermes allowlist and existing cron jobs unchanged. Runtime configuration contains secrets: create private files directly on the Pi and do not paste their contents into Telegram or commit them.
...
2. Install a clean Hermes source checkout and its virtual environment at `/opt/jobhunter-hermes`. The adapter targets the constructor and conversation API verified against Hermes revision `b1ff8722`. The source must contain no owner `.env`, skills, memory or configuration. `jobhunter-auth.service` runs as the owner and resolves the current provider/model and effective credential through Hermes, preserving its original OAuth refresh handling. The restricted service connects through the local peer-authorized `/run/jobhunter-auth/auth.sock`; it never receives refresh tokens or the credential pool. The planner creates a fresh temporary Hermes home and disables all tools, context files, memory and conversation persistence. It fails closed if the resulting agent has tools.
...
4. Use `stacks/jobhunter/service.env.example` as the template for `/etc/jobhunter/service.env`. Set the existing owner bot token, owner Telegram user ID, data root, random local admin token, Hermes paths, `JOBHUNTER_TELEGRAM_MODE=shared` and `JOBHUNTER_OWNER_AUTH_SOCKET=/run/jobhunter-auth/auth.sock`. No separate model key is required. Set ownership `root:jobhunter`, mode `0640`, and use an admin token of at least 32 random characters. Keep `/etc/jobhunter/browser.json` root-owned `0600`, with its data root matching the service. Leave `JOBHUNTER_PUBLIC_URL` empty and browser/Google settings disabled until their HTTPS setup is ready.
...
7. For Google and browser connections, configure a reachable TLS reverse proxy from `stacks/jobhunter/Caddyfile.example`. Forward only the consent callback/connect paths and `/browser/*`; keep `/admin`, `/internal/telegram` and port 8765 on loopback. Do not enable access logs containing connection tokens. The browser has no published ports. A paired relay on `jobhunter_relay` publishes viewer/CDP ports only to loopback and forwards only to its own browser; it has no profile or document mounts. The viewer's secure cookie expires after 30 minutes; revocation closes active WebSockets. A single browser at a time limits Pi memory usage. Its persistent profile and readonly documents mount belong to that candidate only.
8. Install only the tracked `jobhunter-routing` plugin directory beneath the owner's `~/.hermes/plugins/` and enable it in Hermes. Run `python3 tools/patch_jobhunter_gateway.py --repo /home/ala/.hermes/hermes-agent --apply` from the Pi configuration checkout to restore plugin registration after Telegram reconnects on the pinned Hermes revision. The helper checks the revision and exact source before patching; resolve source drift before restarting. Keep `TELEGRAM_ALLOWED_USERS` set to the owner alone and leave allow-all disabled. The plugin consumes all candidate traffic, including unsupported commands and forwarding failures, without passing it to administrative Hermes.
9. Validate the environment with the service interpreter and `python -m jobhunter_service check` under the service's private environment, then explicitly enable/start `jobhunter-auth.service` and `jobhunter.service`. `check` validates required configuration without contacting Telegram or submitting a model request. Verify health locally at `http://127.0.0.1:8765/health` and the authenticated local admin API, then restart the owner's gateway to load the plugin. Confirm collection, private delivery, Google consent and browser login on the actual Pi before claiming those flows are ready; local automated tests alone do not establish deployment readiness.

For a shared HTTPS address without purchasing a domain, use Tailscale Funnel. Install the Pi configuration's `stacks/jobhunter/Caddyfile.funnel` as `/etc/jobhunter/Caddyfile`, root-owned mode `0644`; provision step 70 installs it. Start the isolated browser egress and the loopback gateway with `sudo docker compose -f stacks/jobhunter/compose.yaml --profile https up -d --build`. The gateway listens only on `127.0.0.1:8766` and permits login, consent and browser asset routes; administrative routes and unrelated methods return 404. Both access and HTTP error logging omit requests because connection URLs contain temporary credentials.

After validating the gateway locally, run `sudo tailscale funnel --bg --https=443 http://127.0.0.1:8766`. Complete Tailscale's account approval if prompted. This publishes the gateway using the Pi's `.ts.net` hostname and automatically provisions HTTPS. Set that exact HTTPS origin in private `JOBHUNTER_PUBLIC_URL`, set `JOBHUNTER_BROWSER_ENABLED=true`, and restart `jobhunter.service`. Never target port 8765 directly. The `--bg` configuration persists across Tailscale restarts; the Docker services use `restart: unless-stopped`. To withdraw public access, run `sudo tailscale funnel --bg --https=443 off`; do not reset unrelated Tailscale configuration. Candidates need neither a domain nor Tailscale installed. They request `/connect linkedin` privately and sign in on LinkedIn inside their isolated viewer; passwords and verification codes never belong in Telegram. Google integration remains optional and also needs the web OAuth client from step 5. Verify access through public DNS, one-use links, session expiry, and rejection of administrative routes before onboarding candidates.
...
```json
{"token": "REPLACE_WITH_THE_PRIVATE_ADMIN_TOKEN", "port": 8765, "owner_id": 123456789}
```

Replace the example owner ID with the owner's verified numeric Telegram user ID. Install the tracked `jobhunter-admin` skill and wrapper from the Pi configuration. Owner commands are `python3 ~/.hermes/scripts/jobhunter_admin.py add <telegram_user_id>`, `list`, `suspend <id>` and `revoke <id>`, or their `/jobhunter` Telegram equivalents. They call the authenticated local registry API. Invitation does not send a message to the candidate; the candidate must start a private conversation with the same bot. Do not substitute a channel ID or Telegram display name for a user ID.

For an optional standalone bot, use its own token with `JOBHUNTER_TELEGRAM_MODE=polling` and omit the gateway routing integration. Explicit API-key mode is also supported: omit `JOBHUNTER_OWNER_AUTH_SOCKET` and set `JOBHUNTER_MODEL`, `JOBHUNTER_MODEL_PROVIDER`, `JOBHUNTER_MODEL_API_KEY`, plus `JOBHUNTER_MODEL_BASE_URL` for `custom`. Supported API-key providers are `openai`, `openai-api`, `openrouter`, `anthropic`, `gemini` and `custom`. Never run two pollers for the same Telegram bot.
...
For a new candidate, use `/start` to begin the saved onboarding checklist and `/continue` to resume it later. Resume and experience review, roles and experience limits, destinations and work authorization, schedule, and Telegram preferences precede optional account connections and final activation. The bot shows one next question and explicit review/skip controls. A configuration preview lists the exact proposed values; confirming those settings and acknowledging a reviewed step are separate operations. Uploaded resume facts remain unconfirmed until the candidate accepts their proposed wording. Experience limits show the actual configured bounds; an omitted setting is not described as unlimited. During new-user setup, selecting a time saves a paused schedule; the final activation still requires its own confirmation. Existing active profiles retain their running schedule when viewing status or entering the guide.

LinkedIn checks inspect only whether a signed-in interface is present in that candidate's already running isolated browser; they never enter credentials, navigate, or complete verification. Gmail checks verify the configured mailbox identity, and tracker checks verify account access and the configured Sheet. Checks have a process timeout and return fixed, private messages. Ordinary status reads use bounded, time-stamped evidence and do not make provider calls. Account changes invalidate matching connection evidence; Google consent alone is labelled unverified until checked. If the web OAuth client is missing or invalid for this callback, the candidate sees that owner setup is needed and can skip both Google steps.

Model outages produce a visible notice and retain the candidate's safe request for up to 24 hours. `/retry` allows three explicit attempts against the latest confirmed settings; it never replays a cached proposal or submits an application. A successful outage notice completes that ingress receipt, so `/status`, `/continue` and connection commands can run. Failed Telegram delivery and unexpected mutation failures retain their ordered queue retry behavior. `/support` gives a limited report to the candidate for sharing with the owner; there is no automatic operator notification or candidate access to service logs, code, credentials or system commands. Validate the complete guided dialogue with synthetic profiles before inviting real candidates.

The onboarding transport announces “Please wait” before resume extraction, model calls and explicit connection checks, then marks questions, settings reviews and login instructions as “Your turn.” A newly confirmed resume change starts one automatic follow-up question while the resume step is active. The model receives the confirmed profile and conversation and may only reply in that follow-up; it cannot propose another mutation or complete the review. Repeated confirmation clicks do not restart the interview. A failed follow-up has its own retry intent, which resumes from current confirmed facts and stops if the candidate has already moved to another setup step. Candidates choose “Done reviewing” explicitly.

Experience thresholds live in `search.matching.seniority`: `min_years`, `max_years`, `preferred_min_years` and `preferred_max_years`. Candidates can request these in ordinary messages; unknown advertised experience remains unknown. Optional `telegram.presentation` accepts `style` (`standard` or `compact`), `show_salary`, `show_match_reason` and `group_by_market` booleans. For example, “Use a compact digest, group jobs by destination and show the advertised salary” produces a preview for confirmation. Omitted preferences preserve the existing layout. Salary and reasons come from listing/review data; unavailable values are identified explicitly. Candidates need no personal website: the HTTPS login and consent links use the operator's shared service address.
