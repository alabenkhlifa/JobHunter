...
def verify_account(drive, account: str) -> None:
    ...
    if actual.casefold() != account.casefold():
        raise GmailAuthError("Authorized Drive account does not match the configured jobs account; access stopped.")
...
def checked_services(token_path: Path, account: str | None = None, *, force_refresh: bool = False):
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "authorize":
            authorize(args)
        sheets, _ = checked_services(args.google_token, args.account, force_refresh=args.refresh)
